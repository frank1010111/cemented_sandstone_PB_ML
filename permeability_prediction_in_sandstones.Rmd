...
accuracy_CK <- df3 %>%
  select(Permeability, `Compaction & cements`, `Porosity group`) %>%
  rename(Estimate=`Compaction & cements`) %>%
  dplyr::group_by(`Porosity group`) %>%
  dplyr::summarise(Rsquared = round( (cor( log(Permeability), log(Estimate)) ^ 2), 2),
                   RMSE = round(sqrt(mean((log(Permeability) - log(Estimate)) ^ 2)),2),
                   MAE = round(mean(abs(log(Permeability) - log(Estimate))),2 ))
accuracy_CK_test <- df_holdout %>%
  mutate(Estimate = (CK_void_fraction / (2 * tau_e * a_e_sorta^2))) %>%
  rename(`Porosity group` = porosity_group, Permeability=KLH) %>%
  dplyr::group_by(`Porosity group`) %>%
  dplyr::summarise(Rsquared = round( (cor( log(Permeability), log(Estimate)) ^ 2), 2),
                   RMSE = round(sqrt(mean((log(Permeability) - log(Estimate)) ^ 2)),2),
                   MAE = round(mean(abs(log(Permeability) - log(Estimate))),2 ))
...
```{r model_fitness, fig.width=5.5, fig.height=5}
...
model_fitness <- 
  rbind(
    ...
    accuracy_CK[ c("RMSE", "Rsquared", "MAE") ],
    accuracy_CK_test[ c("RMSE", "Rsquared", "MAE") ]
  ) %>%
  as_tibble() %>%
  round(2) %>%
  rename(`R^2^`=Rsquared)
...
model_fitness$Model <- c("Gradient boosting","Gradient boosting","Gradient boosting","Gradient boosting",
                         ...
                         "Carman-Kozeny", "Carman-Kozeny","Carman-Kozeny", "Carman-Kozeny")
...
model_fitness[,"Data"] <- c("Train","Test","Train","Test",
                         "Train","Test","Train","Test", "Train", "Train", "Test","Test")
...
p1 <- model_fitness %>%
  ggplot(aes(x=Model, y=MAE, fill=`Porosity group`)) +
  facet_wrap(~Data) +
  coord_flip() +
...
p2 <- 
  model_fitness %>%
  ggplot(aes(x=Model, y=RMSE, fill=`Porosity group`)) +
  facet_wrap(~Data) +
  coord_flip() +
  geom_col(position="dodge") +
  scale_fill_manual(values = c("peru","steelblue")) +
...
p3 <- 
  model_fitness %>%
  ggplot(aes(x=Model, y=`R^2^`, fill=`Porosity group`)) +
  facet_wrap(~Data) +
  coord_flip() +
  geom_col(position="dodge") +
  scale_fill_manual(values = c("peru","steelblue")) +
...
#model_fitness %>%
#  ggplot(aes(x=Data, y=`R^2^`, fill=`Porosity group`)) +
#  facet_wrap(~Model) +
#  coord_flip() +
#  geom_col(position="dodge")
ggsave("figures/R2_comparison.png", p3, width=7, height=2.5, dpi=300)

```
```{r ttests}

# #low porosity -training
# t.test((resid(fit_xgboost_lp)^2), 
#        (resid(fit_enet_lp)^2), 
#        paired=TRUE,
#        alternative="two.sided"
#        )
# t.test(abs(resid(fit_xgboost_lp)), 
#        abs(resid(fit_enet_lp)), 
#        paired=TRUE,
#        alternative="two.sided"
#        )
# # low porosity - testing
# t.test((resid(fit_xgboost_lp, df_holdout_lp)^2), 
#        (resid(fit_enet_lp, test_lp)^2), 
#        paired=TRUE,
#        alternative="two.sided"
#        )
# t.test(abs(resid(fit_xgboost_lp, df_holdout_lp)), 
#        abs(resid(fit_enet_lp, test_lp)), 
#        paired=TRUE,
#        alternative="two.sided"
#        )
# 
# #high porosity -training
# t.test((resid(fit_xgboost_hp)^2), 
#        (resid(fit_enet_hp)^2), 
#        paired=TRUE,
#        alternative="less"
#        )
# t.test(abs(resid(fit_xgboost_hp)), 
#        abs(resid(fit_enet_hp)), 
#        paired=TRUE,
#        alternative="less"
#        )
# 
# # high porosity - testing
# t.test((resid(fit_xgboost_hp, df_holdout_hp)^2), 
#        (resid(fit_enet_hp, test_hp)^2), 
#        paired=TRUE,
#        alternative="less"
#        )
# t.test(abs(resid(fit_xgboost_hp, df_holdout_hp)), 
#        abs(resid(fit_enet_hp, test_hp)), 
#        paired=TRUE,
#        alternative="less"
#        )
```
