...
df <- readWorksheetFromFile("data/Garn1990.xlsx", sheet = 1) %>%
  mutate_at(vars(POR:IMP), ~ as.numeric(.)) %>%
  mutate(WELL = str_trim(WELL)) %>%
  # Replace zeros in interparticle porosity and cement content to a value
  # below the lowest measurable quantity
  mutate(
...
df <- df %>%
  ...
  mutate(
    k_pl94_por = (mean_GS^2 * POR^3) / (72 * tau_u * (1 - POR)^2) * (gamma_GS * Cv_GS^3 + 3 * Cv_GS^2 + 1)^2 / (1 + Cv_GS^2)^2,
...
fit_control_lp <- trainControl(
  index = groupKFold(train_lp$WELL),
  allowParallel = TRUE
)
fit_control_hp <- trainControl(
  index = groupKFold(train_hp$WELL),
  allowParallel = TRUE
)
```
...
```{r correlations}
df2 <- data.table::as.data.table(df)[, list(
  Permeability = KLH,
...
df2 %>%
  melt(measure.vars = c(
    "Carman-Kozeny void fraction", "Tortuosity", "Specific surface area", "Pore-bridging cement",
    "Pore-filling cement"
  )) %>%
  drop_na() %>%
  group_by(variable, `Porosity group`) %>%
  dplyr::summarize(
    `Pearson r` = round(cor(log(Permeability), log(value)), 2),
...
reg_plot <- function(df, kpred) {
  ...
  p <-
    ...
    scale_x_log10("Actual permeability (mD)",
      breaks = c(0.1, 1, 10, 100, 1000, 1e4),
...
df3 <- data.table::as.data.table(df)[, list(
  Permeability = KLH,
...
p4 <-
  ...
  group_by(variable, `Porosity group`) %>%
  dplyr::summarize(
    `Pearson r` = round((cor(log(Permeability), log(value))^2), 2)
    # `Kendall tau` = round(cor(Permeability, value, method = "kendall"), 2)
  ) %>%
...
p1 <-
  ...
  scale_color_manual("Porosity\ngroup", values = c("peru", "steelblue")) + # = c("black", "black")) +
  annotation_logticks(sides = "bl") +
...
# ggsave("predict_compare.png", p1, width=7, height=3.5)
p2 <-
  bind_rows(
    df_lowphi %>%
      mutate(
...
data <- rbind(
  merge(
    melt(shaps_lp$shap_contrib, value.name = "SHAP"),
    melt(shaps_lp$data, value.name = "Value")
  ) %>%
    mutate(`Porosity group` = "Low"),
  merge(
    melt(shaps_hp$shap_contrib, value.name = "SHAP"),
...
<!--$$\begin{align}-->
$$k = \left[\bar{D}^2 \phi^3 \left(\gamma C_D^3+3C_D^2+1\right)^2\right]\times$$ $$\left\{2\tau_e (1-\phi)^2 \left[ 6\left(1+C_D^2\right)\frac{1-\phi_u}{1-\phi}+\left(a_b P_b + a_f P_f \right) \bar{D} \left(\gamma C_D^3 + 3C_D^2 +1 \right) \right]^2\right\}^{-1}$$
 <!--\end{align}$$-->
